...
var MODES = {
    SINGLE: { lore: "§6§l当前模式: §7单个清除", msg: "§a已清除最近的全息物品" },
    RANGE:  { lore: "§6§l当前模式: §b范围清除", msg: "§a已清除周围20x20x20区域内的全息物品" }
};
...
function onUse(e) {
    ...
    if (mode === MODES.SINGLE) {
        ...
        for (var ent of world.getEntities()) {
            if (ent instanceof ItemDisplay && !ent.isDead()) {
        ...
        p.sendMessage("§a已清除 " + count + " 个全息物品" + (skipped > 0 ? "，跳过 " + skipped + " 个无权限的" : ""));
        if (skipped > 0) {
            p.playSound(loc, "block.note_block.bass", 1.0, 0.5);
        } else if (count > 0) {
