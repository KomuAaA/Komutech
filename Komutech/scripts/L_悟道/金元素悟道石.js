...
// 配置常量
const WUDAO_STONE = "KOMUTECH_L_WD_JYSWDS";
const TARGET_MACHINE_ID = "KOMUTECH_L_JQ_FZCZQ";
const TARGET_ITEM_ID = "KOMUTECH_L_FZ_YSJYSFZ";
...
function processPlayer(loc, player, now, state) {
    ...
    if (!timer) {
        const furnaces = findFurnacesWithIron(loc);
        ...
        state.timer = {
            ...
            startTime: now,
            furnaces: furnaces,
            maxItems: Math.min(BASE_MAX_ITEMS + furnaces.length * BLOCK_BONUS, MAX_ITEMS_LIMIT)
        };
        
        player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟到${furnaces.length}个高炉，开始悟道！`);
        return;
    }
    ...
    if (!timer.hasConverted && checkBlocksDestroyed(timer.furnaces)) {
        player.sendMessage(ChatColor.RED + "[悟道石] 粗铁块被破坏，悟道中断！");
        delete state.timer;
        return;
    }
    
    if (!timer.hasConverted && elapsed >= GENERATION_TIME / 2) {
        convertIronBlocks(timer.furnaces);
        timer.hasConverted = true;
    ...
    if (!timer.generated && elapsed >= GENERATION_TIME) {
        generateItems(loc, player, timer.maxItems, timer.furnaces.length);
...
function findFurnacesWithIron(loc) {
    const world = loc.getWorld();
    ...
    for (let dx = -2; dx <= 2; dx++) {
        for (let dy = -1; dy <= 2; dy++) {
            for (let dz = -2; dz <= 2; dz++) {
                if (world.getBlockAt(x + dx, y + dy, z + dz).getType() === Material.BLAST_FURNACE &&
                    world.getBlockAt(x + dx, y + dy + 1, z + dz).getType() === Material.RAW_IRON_BLOCK) {
                    furnaces.push({
                        world: world,
                        x: x + dx,
                        y: y + dy + 1,
...
function checkBlocksDestroyed(furnaces) {
    for (let furnace of furnaces) {
        const block = furnace.world.getBlockAt(furnace.x, furnace.y, furnace.z);
        const expectedType = furnace.converted ? Material.IRON_BLOCK : Material.RAW_IRON_BLOCK;
...
function convertIronBlocks(furnaces) {
    for (let furnace of furnaces) {
        const block = furnace.world.getBlockAt(furnace.x, furnace.y, furnace.z);
        if (block.getType() === Material.RAW_IRON_BLOCK) {
            block.setType(Material.IRON_BLOCK);
            furnace.converted = true;
        }
    }
}

// 物品生成
function generateItems(wudaoLoc, player, maxItems, furnaceCount) {
    ...
    player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟 ${count} 个物品`);
}
