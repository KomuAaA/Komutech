...
// 配置常量
const WUDAO_STONE = "KOMUTECH_L_WD_TYSWDS";
const TARGET_MACHINE_ID = "KOMUTECH_L_JQ_FZCZQ";
const TARGET_ITEM_ID = "KOMUTECH_L_FZ_YSTYSFZ";
...
const DIRT_BLOCKS = [
    Material.DIRT,              // 泥土
    Material.GRASS_BLOCK,       // 草方块
    Material.COARSE_DIRT,       // 灰化土
    Material.DIRT_PATH,         // 泥巴
    Material.PODZOL,            // 砂土
    Material.MYCELIUM,          // 菌丝
    Material.ROOTED_DIRT,       // 缠根泥土
    Material.MUD,               // 泥巴
    Material.FARMLAND           // 耕地
];
...
function processPlayer(loc, player, now, state) {
    ...
    if (!timer) {
        const dirtTypes = findAndClearDirtBlocks(loc);
        const dirtTypeCount = dirtTypes.size;
        
        if (dirtTypeCount === 0) return;
        
        state.timer = {
            playerId: player.getUniqueId().toString(),
            startTime: now,
            maxItems: Math.min(BASE_MAX_ITEMS + dirtTypeCount * DIRT_BONUS, MAX_ITEMS_LIMIT),
            dirtTypeCount: dirtTypeCount
        };
        
        player.sendMessage(ChatColor.GREEN + `[悟道石] 感应${dirtTypeCount}种泥土类方块，开始悟道！`);
        return;
    }
    ...
    if (!timer.generated && elapsed >= GENERATION_TIME) {
        generateItems(loc, player, timer.maxItems, timer.dirtTypeCount);
    ...
    for (let x = centerX - 2; x <= centerX + 2; x++) {
        for (let y = centerY - 1; y <= centerY + 1; y++) {
            for (let z = centerZ - 2; z <= centerZ + 2; z++) {
                ...
                if (isDirtBlock(type)) {
                    dirtTypes.add(type.name()); // 使用方块名称作为类型标识
...
function isDirtBlock(material) {
    for (let i = 0; i < DIRT_BLOCKS.length; i++) {
        if (material === DIRT_BLOCKS[i]) {
            return true;
        }
    }
    return false;
...
// 物品生成
function generateItems(wudaoLoc, player, maxItems, dirtTypeCount) {
    ...
    player.sendMessage(ChatColor.GREEN + `[悟道石] 感应${dirtTypeCount}种泥土类方块，生成 ${count} 个物品`);
}
