...
// 配置常量
const WUDAO_STONE = "KOMUTECH_L_WD_BYSWDS";
const TARGET_MACHINE_ID = "KOMUTECH_L_JQ_FZCZQ";
const TARGET_ITEM_ID = "KOMUTECH_L_FZ_YSBYSFZ";
...
function processPlayer(loc, player, now, state) {
    ...
    if (!timer) {
        const iceTypeCount = findAndClearIces(loc);
        if (iceTypeCount === 0) return;
        
        state.timer = {
            playerId: player.getUniqueId().toString(),
            startTime: now,
            maxItems: Math.min(BASE_MAX_ITEMS + iceTypeCount * ICE_BONUS, MAX_ITEMS_LIMIT),
            iceTypeCount: iceTypeCount
        };
        
        player.sendMessage(ChatColor.AQUA + `[悟道石] 感悟${iceTypeCount}种，开始悟道！`);
        return;
    }
...
function findAndClearIces(loc) {
    ...
    return iceTypes.size;
}

// 物品生成
function generateItems(wudaoLoc, player, maxItems, iceTypeCount) {
    ...
    player.sendMessage(ChatColor.AQUA + "[悟道石] 我悟了！！！");
    player.sendMessage(ChatColor.WHITE + `[悟道石] 感悟${iceTypeCount}种，生成 ${count} 个物品`);
}
...
function onPlace(event) {
    event.getPlayer().sendMessage(ChatColor.AQUA + "✓ 已放置悟道石");
}
