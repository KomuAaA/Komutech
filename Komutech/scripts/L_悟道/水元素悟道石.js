...
// 配置常量
const WUDAO_STONE = "KOMUTECH_L_WD_SYSWDS";
const TARGET_MACHINE_ID = "KOMUTECH_L_JQ_FZCZQ";
const TARGET_ITEM_ID = "KOMUTECH_L_FZ_YSSYSFZ";
...
function processPlayer(loc, player, now, state) {
    ...
    if (!timer) {
        const flowingWaterCount = findAndReplaceFlowingWater(loc);
        if (flowingWaterCount === 0) return;
        
        state.timer = {
            playerId: player.getUniqueId().toString(),
            startTime: now,
            maxItems: Math.min(BASE_MAX_ITEMS + flowingWaterCount * WATER_BONUS, MAX_ITEMS_LIMIT),
            flowingWaterCount: flowingWaterCount
        };
        
        player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${flowingWaterCount}处流水，开始悟道！`);
        return;
    }
    ...
    if (!timer.generated && elapsed >= GENERATION_TIME) {
        generateItems(loc, player, timer.maxItems, timer.flowingWaterCount);
...
function findAndReplaceFlowingWater(loc) {
    const world = loc.getWorld();
    ...
    return flowingWaterCount;
}

// 物品生成
function generateItems(wudaoLoc, player, maxItems, flowingWaterCount) {
    ...
    player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${flowingWaterCount}处流水，生成 ${count} 个物品`);
}
