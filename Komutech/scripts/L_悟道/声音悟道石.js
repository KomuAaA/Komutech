...
// 配置常量
const WUDAO_STONE = "KOMUTECH_L_WD_SYWDS";
const TARGET_MACHINE_ID = "KOMUTECH_L_JQ_FZCZQ";
const TARGET_ITEM_ID = "KOMUTECH_L_FZ_YSSYFZ";
...
function processPlayer(loc, player, now, state) {
    ...
    if (!timer) {
        const wardens = findAndClearWardens(loc);
        if (wardens.length === 0) return;
        
        state.timer = {
            playerId: player.getUniqueId().toString(),
            startTime: now,
            maxItems: Math.min(BASE_MAX_ITEMS + wardens.length * WARDEN_BONUS, MAX_ITEMS_LIMIT),
            wardenCount: wardens.length
        };
        
        player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${wardens.length}个监守者，开始悟道！`);
        return;
    }
    ...
    if (!timer.generated && elapsed >= GENERATION_TIME) {
        generateItems(loc, player, timer.maxItems, timer.wardenCount);
...
function findAndClearWardens(loc) {
    const world = loc.getWorld();
    const center = loc.clone().add(0.5, 1, 0.5);
    const wardens = [];
...
// 物品生成
function generateItems(wudaoLoc, player, maxItems, wardenCount) {
    ...
    player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${wardenCount}个监守者，感悟 ${count} 个物品`);
}
