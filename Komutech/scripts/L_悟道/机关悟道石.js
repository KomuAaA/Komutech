...
// 配置常量
const WUDAO_STONE = "KOMUTECH_L_WD_JGWDS";
const TARGET_MACHINE_ID = "KOMUTECH_L_JQ_FZCZQ";
const TARGET_ITEM_ID = "KOMUTECH_L_FZ_YSJGFZ";
...
function processPlayer(loc, player, now, state) {
    ...
    if (!timer) {
        const golems = findAndClearIronGolems(loc);
        if (golems.length === 0) return;
        
        state.timer = {
            playerId: player.getUniqueId().toString(),
            startTime: now,
            maxItems: Math.min(BASE_MAX_ITEMS + golems.length * GOLEM_BONUS, MAX_ITEMS_LIMIT),
            golemCount: golems.length
        };
        
        player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${golems.length}个铁傀儡，开始悟道！`);
        return;
    }
    ...
    if (!timer.generated && elapsed >= GENERATION_TIME) {
        generateItems(loc, player, timer.maxItems, timer.golemCount);
...
function findAndClearIronGolems(loc) {
    const world = loc.getWorld();
    const center = loc.clone().add(0.5, 1, 0.5);
    const golems = [];
...
// 物品生成
function generateItems(wudaoLoc, player, maxItems, golemCount) {
    ...
    player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${golemCount}个铁傀儡，感悟 ${count} 个物品`);
}
