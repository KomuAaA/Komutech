...
// 配置常量
const WUDAO_STONE = "KOMUTECH_L_WD_MYSWDS";
const TARGET_MACHINE_ID = "KOMUTECH_L_JQ_FZCZQ";
const TARGET_ITEM_ID = "KOMUTECH_L_FZ_YSMYSFZ";
...
function processPlayer(loc, player, now, state) {
    ...
    if (!timer) {
        const saplingTypeCount = findAndClearSaplings(loc);
        if (saplingTypeCount === 0) return;
        
        state.timer = {
            playerId: player.getUniqueId().toString(),
            startTime: now,
            maxItems: Math.min(BASE_MAX_ITEMS + saplingTypeCount * SAPLING_BONUS, MAX_ITEMS_LIMIT),
            saplingTypeCount: saplingTypeCount
        };
        
        player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${saplingTypeCount}种树苗，开始悟道！`);
        return;
    }
    ...
    if (!timer.generated && elapsed >= GENERATION_TIME) {
        generateItems(loc, player, timer.maxItems, timer.saplingTypeCount);
...
function findAndClearSaplings(loc) {
    ...
    for (let x = centerX - 2; x <= centerX + 2; x++) {
        for (let y = centerY - 1; y <= centerY + 1; y++) {
            for (let z = centerZ - 2; z <= centerZ + 2; z++) {
                ...
                if (name.includes("SAPLING") && name !== "DEAD_BUSH") {
                    saplingTypes.add(name);
    ...
    return saplingTypes.size;
}

// 物品生成
function generateItems(wudaoLoc, player, maxItems, saplingTypeCount) {
    ...
    player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${saplingTypeCount}种树苗，生成 ${count} 个物品`);
}
