...
// 配置常量
const WUDAO_STONE = "KOMUTECH_L_WD_LYSWDS";
const TARGET_MACHINE_ID = "KOMUTECH_L_JQ_FZCZQ";
const TARGET_ITEM_ID = "KOMUTECH_L_FZ_YSLYSFZ";
const TLY_ITEM_ID = "KOMUTECH_L_KW_TLY";
...
function processPlayer(loc, player, now, state) {
    ...
    if (!timer) {
        const rodCount = findAndRemoveLightningRods(loc);
        if (rodCount === 0) return;
        
        state.timer = {
            playerId: player.getUniqueId().toString(),
            startTime: now,
            maxItems: Math.min(BASE_MAX_ITEMS + rodCount * ROD_BONUS, MAX_ITEMS_LIMIT),
            rodCount: rodCount
        };
        
        player.sendMessage(ChatColor.DARK_PURPLE + `[悟道石] 检测到${rodCount}个避雷针并已移除，开始悟道！`);
        return;
    }
    ...
    if (!timer.generated && elapsed >= GENERATION_TIME) {
        generateItems(loc, player, timer.maxItems, timer.rodCount);
...
function findAndRemoveLightningRods(loc) {
    const world = loc.getWorld();
...
// 物品生成
function generateItems(wudaoLoc, player, maxItems, rodCount) {
    ...
    // 检查霆雷玉
    if (!findAndConsumeTLY(menu)) {
        player.sendMessage(ChatColor.RED + "[悟道石] 目标机器中没有霆雷玉！");
        return;
    }
    ...
    player.sendMessage(ChatColor.LIGHT_PURPLE + `[悟道石] 通过${rodCount}个避雷针感悟1个霆雷玉，生成 ${count} 个物品`);
}

// 查找并消耗霆雷玉
function findAndConsumeTLY(menu) {
    const contents = menu.getContents();
    
    for (let slot = 0; slot < contents.length; slot++) {
        const item = contents[slot];
        if (item && isSlimefunItem(item, TLY_ITEM_ID)) {
            if (item.getAmount() > 1) {
...
// 检查是否为指定Slimefun物品
function isSlimefunItem(itemStack, itemId) {
    try {
        const sfItem = Java.type("io.github.thebusybiscuit.slimefun4.api.items.SlimefunItem").getByItem(itemStack);
        return sfItem && sfItem.getId() === itemId;
    } catch (e) {
        return false;
    }
}
