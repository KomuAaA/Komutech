...
// 配置常量
const WUDAO_STONE = "KOMUTECH_L_WD_LJWDS";
const TARGET_MACHINE_ID = "KOMUTECH_L_JQ_FZCZQ";
const TARGET_ITEM_ID = "KOMUTECH_L_FZ_YSLJFZ";
...
function processPlayer(loc, player, now, state) {
    ...
    if (!timer) {
        const witches = findAndClearWitches(loc);
        if (witches.length === 0) return;
        
        state.timer = {
            playerId: player.getUniqueId().toString(),
            startTime: now,
            maxItems: Math.min(BASE_MAX_ITEMS + witches.length * WITCH_BONUS, MAX_ITEMS_LIMIT),
            witchCount: witches.length
        };
        
        player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${witches.length}个女巫，开始悟道！`);
        return;
    }
    ...
    if (!timer.generated && elapsed >= GENERATION_TIME) {
        generateItems(loc, player, timer.maxItems, timer.witchCount);
...
function findAndClearWitches(loc) {
    const world = loc.getWorld();
    const center = loc.clone().add(0.5, 1, 0.5);
    const witches = [];
...
// 物品生成
function generateItems(wudaoLoc, player, maxItems, witchCount) {
    ...
    player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${witchCount}个女巫，感悟 ${count} 个物品`);
}
