...
// 配置常量
const WUDAO_STONE = "KOMUTECH_L_WD_FYSWDS";
const TARGET_MACHINE_ID = "KOMUTECH_L_JQ_FZCZQ";
const TARGET_ITEM_ID = "KOMUTECH_L_FZ_YSFYSFZ";
...
function processPlayer(loc, player, now, state) {
    ...
    if (!timer) {
        const breezes = findAndClearBreezes(loc);
        if (breezes.length === 0) return;
        
        state.timer = {
            playerId: player.getUniqueId().toString(),
            startTime: now,
            maxItems: Math.min(BASE_MAX_ITEMS + breezes.length * BREEZE_BONUS, MAX_ITEMS_LIMIT),
            breezeCount: breezes.length
        };
        
        player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${breezes.length}个旋风人，开始悟道！`);
        return;
    }
    ...
    if (!timer.generated && elapsed >= GENERATION_TIME) {
        generateItems(loc, player, timer.maxItems, timer.breezeCount);
...
function findAndClearBreezes(loc) {
    const world = loc.getWorld();
    const center = loc.clone().add(0.5, 1, 0.5);
    const breezes = [];
...
// 物品生成
function generateItems(wudaoLoc, player, maxItems, breezeCount) {
    ...
    player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${breezeCount}个旋风人，感悟 ${count} 个物品`);
}
