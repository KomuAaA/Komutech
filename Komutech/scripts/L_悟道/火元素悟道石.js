...
// 配置常量
const WUDAO_STONE = "KOMUTECH_L_WD_HYSWDS";
const TARGET_MACHINE_ID = "KOMUTECH_L_JQ_FZCZQ";
const TARGET_ITEM_ID = "KOMUTECH_L_FZ_YSHYSFZ";
...
function processPlayer(loc, player, now, state) {
    ...
    if (!timer) {
        const fireCount = findAndExtinguishFires(loc);
        if (fireCount === 0) return;
        
        state.timer = {
            playerId: player.getUniqueId().toString(),
            startTime: now,
            maxItems: Math.min(BASE_MAX_ITEMS + fireCount * FIRE_BONUS, MAX_ITEMS_LIMIT),
            fireCount: fireCount
        };
        
        player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${fireCount}团火焰，开始悟道！`);
        return;
    }
    ...
    if (!timer.generated && elapsed >= GENERATION_TIME) {
        generateItems(loc, player, timer.maxItems, timer.fireCount);
...
function findAndExtinguishFires(loc) {
    const world = loc.getWorld();
...
// 物品生成
function generateItems(wudaoLoc, player, maxItems, fireCount) {
    ...
    player.sendMessage(ChatColor.GREEN + `[悟道石] 感悟${fireCount}团火焰，感悟 ${count} 个物品`);
}
